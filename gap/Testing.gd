...
#! @Description Converts the modular data (S,T), which must have level dividing p^ld,
...
#! @Arguments p, lambda
#! @Returns a boolean
#! @Description Constructs and tests all irreps of level dividing p^ld by checking their
#! positions in Irr(G).
