...
#----------------------------------------
# Returns conj class representative information of SL(2, Z/p^ld Z) for ld (>= 1) using CCOdd or CCEven.
#---------------------------------------

CClasses:=function (p, ld)
	if p = 2 then 
		return CCEven(ld);
	elif p > 2 then
		return CCOdd(p,ld);
