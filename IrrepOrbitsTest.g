...
PositionTest := function(irreps, rho, level, pos_list)
    local pos_1, pos, k, conductor;

    pos_1 := Position(irreps, ChiST(rho[1], rho[2]));
    if pos_1 = fail then
        # Didn't find rho itself; no point continuing
        Print("FAILED!\n");
        return;
    fi;
    Print(" (deg ", rho[3], ")\n\tid: ", pos_1, "\t");
    ...
    # Tensor products of rho with 1-dimensional reps. C_2, C_3, C_4.
    if 2 < level and level mod 2 = 0 then
        # C2 tensor rho
        pos := Position(irreps, ChiST(-1 * rho[1], -1 * rho[2]));
        if pos_1 <> pos then
            Print("C2: ", pos, "\t");
        fi;
    fi;
    if 4 < level and level mod 4 = 0 then
        # C3 tensor rho
        pos := Position(irreps, ChiST(E(4) * rho[1], -E(4) * rho[2]));
        if pos_1 <> pos then
            Print("C3: ", pos, "\t");
        fi;
    ...
    if 3 < level and level mod 3 = 0 then
        # R_1(1)- tensor rho
        pos := Position(irreps, ChiST(rho[1], E(3) * rho[2]));
        if pos_1 <> pos then
            Print("R1: ", pos, "\t");
        fi;

        # R_1(2)- tensor rho
        pos := Position(irreps, ChiST(rho[1], (E(3)^2) * rho[2]));
        if pos_1 <> pos then
            Print("R2: ", pos, "\t");
        fi;
    fi;
    ...
    # Images under Galois action
    # Find the conductor of S and T
    # maybe use level * (p+1) * (p-1) * 12?
    conductor := 1;
    for k in [1 .. Length(rho[1])] do
        conductor := Lcm(conductor, Conductor(rho[1][k]));
    od;
    for k in [1 .. Length(rho[2])] do
        conductor := Lcm(conductor, Conductor(rho[2][k][k]));
    od;
    Print("w = ", conductor, ".\t");
    if conductor > 1 then
        for k in PrimeResidues(conductor) do
            # Apply the element of Gal(C) given by zeta |-> zeta^k.
            if k = 1 then
                # trivial
                continue;
            fi;
            # if conductor mod 2 = 0 and k > conductor / 2 then
            #     # zeta |-> zeta^(conductor / 2) = zeta |-> -zeta, which is
            #     # the same as tensoring with C2, which we will already have done.
            #     break;
            # fi;
            pos := Position(irreps, ChiST(GaloisCyc(rho[1], k), GaloisCyc(rho[2], k)));
            if pos_1 <> pos then
                Print("s", k,": ", pos, "\t");
            fi;
        od;
    ...
    if ld = 1 then
        ...
        if p > 3 then
            for i in [1 .. (p-3)/2] do
                ...
                Print("D_1([", i, ",0])");
                PositionTest(irreps, rho, l, pos_list);
            od;
        fi;
        ...
        Print("R_1(1)+");
        PositionTest(irreps, rho[1], l, pos_list);
        Print("R_1(1)-");
        PositionTest(irreps, rho[2], l, pos_list);
